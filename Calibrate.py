...
def CamCalibrate(w, h, num, root):
    ...
    for i in range(num):
        ...
        # 这里世界坐标系的人为定义的坐标与像素坐标系的像素建立换算关系是通过u,U(X),V(Y)和v,U(X),V(Y)建立约束方程建立的见(实际上这个单应性变换矩阵完成了人为定义的度量坐标与像素坐标的转换) https://zhuanlan.zhihu.com/p/94244568，因此最终所有的物理量都是由像素表示的当然反过来也可以都用人为定义的这个世界坐标系的度量单位衡量
